...
# Canonical common feedback runtime.
p = Path('olli-feedback-runtime.js')
s = p.read_text(encoding='utf-8')
start = s.index('  function wrapFunction(name, validator) {')
end = s.index('  /* onclick보다 먼저', start)
s = s[:start] + "  /* 입력 검증은 각 canonical request 함수가 직접 소유한다.\n   이 블록은 버튼 capture 단계의 조기 안내만 담당하며 전역 함수를 교체하지 않는다. */\n\n" + s[end:]
...
s = once(s,
    ...
    'archive edit feature hook')
if 'function wrapFunction(name, validator)' in s or 'installFeedbackInputGuards' in s:
    raise SystemExit('feedback wrapper guard remains')
p.write_text(s, encoding='utf-8')


# Canonical registration runtime: selected student identity is separate from feedback body.
p = Path('olli-feedback-registration-runtime.js')
s = p.read_text(encoding='utf-8')
old_helper = """  function getKcfSelectedStudentForName(studentName){
...
"""
new_helper = """  function getKcfSelectedStudent(){
    var selectedId = '';
    try {
...
"""
s = once(s, old_helper, new_helper, 'selected student helper')

tail_start = s.index('  window.selectKinderChatFeedbackStudentFromManage = function')
tail_end = s.rfind('})();')
if tail_end < tail_start:
    raise SystemExit('registration tail markers missing')
new_tail = r'''  window.selectKinderChatFeedbackStudentFromManage = function(studentId, event){
    if (event) {
...
s = s[:tail_start] + new_tail + s[tail_end + len('})();'):]
if '첫 줄에는 학생 이름을' in s:
    raise SystemExit('legacy first-line rule remains')
if 'parseKinderChatFeedbackInput(text)' in s:
    raise SystemExit('legacy name-first parser remains in submit path')
p.write_text(s, encoding='utf-8')


Path('tests/one-minute-feedback-common.test.cjs').write_text(r'''const test = require('node:test');
