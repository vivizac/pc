...
- 일반 팀톡으로 보낸 내 텍스트에 날짜·요일·시간 중 서로 다른 2종류 이상이 포함되면 작은 `올리 응답` 버튼을 표시한다.
- 날짜 신호는 오늘/내일, 월·일 또는 일자 표현이며, `다음주 화요일`처럼 주 범위+요일도 날짜+요일 조합으로 인정한다.
- 버튼을 누른 경우에만 기존 명령 라우터의 `runSuggestedQuery()`가 원문을 시간표 조회 힌트와 함께 해석한다. 일반 채팅 자체를 자동으로 올리 명령으로 바꾸지 않는다.
- 기존 올리 AI/봇 설정을 그대로 따르며, 응답은 원문 메시지의 `reply_to_message_id`로 저장한다. 이미 올리 응답이 연결된 메시지는 새로고침 후에도 버튼을 다시 표시하지 않는다.
- DB/RPC는 변경하지 않고 기존 팀톡 메시지의 `reply_to_message_id`를 그대로 사용한다. Phone UI는 플랫폼 전용 렌더러에서 같은 공통 판별 함수를 사용한다.
