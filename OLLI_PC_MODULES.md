...
- 초등부 1시 칸은 학생 4명이 들어가는 최소 높이를 사용하고, 4명을 넘으면 행 높이가 자동으로 늘어나 모든 학생을 표시한다.
- 초등부의 `클래스 분리`는 학원·요일·시간 단위로 저장한다. 분리된 칸은 별도 박스나 A·B 텍스트 없이 가로선 하나로 위(A)·아래(B)를 나누며, 기존 학생은 A반에 유지한다. 분리된 시간으로 수업을 옮기거나 추가할 때 팝업에서 A반·B반을 선택한다.
