...
(function installOlliStorageRetryBridge(global) {
  ...
  const TARGETS = Object.freeze({
    general_feedback: { table: 'feedbacks', rpc: 'olli_feedback_insert_idempotent', label: '일반 피드백' },
    growth_feedback: { table: 'fail_feedbacks', rpc: 'olli_growth_feedback_insert_idempotent', label: '성장 피드백' },
    summary_feedback: { table: 'summary_feedbacks', rpc: 'olli_summary_feedback_insert_idempotent', label: '종합 피드백' }
  });
  ...
  function currentAcademyId(payload) {
    ...
    try { return clean(localStorage.getItem('olli_current_academy_id')); } catch (_) { return ''; }
  }
  function pendingKey(academyId) { return `${PENDING_KEY_PREFIX}_${clean(academyId) || 'unscoped'}`; }
  ...
  function writePending(academyId, list) {
    try { localStorage.setItem(pendingKey(academyId), JSON.stringify(Array.isArray(list) ? list.slice(0, 80) : [])); }
    catch (error) { console.warn('피드백 재전송 pending 정리 실패:', error?.message || error); }
  }
  ...
  function findPendingForPayload(tableName, payload) {
    ...
    return readPending(academyId).find(item => sameFeedbackPayload(item, tableName, payload)) || null;
  }
  function clearPendingMutation(academyId, mutationId) {
    const id = clean(mutationId);
    if (!academyId || !id) return;
    writePending(academyId, readPending(academyId).filter(item => clean(item.mutationId) !== id));
  }
  ...
  function clearGenericQueueMutation(academyId, feature, mutationId) {
    ...
    queue.read(academyId)
      .filter(item => clean(item.feature) === clean(feature) && clean(item.client_mutation_id) === clean(mutationId))
      .forEach(item => queue.remove(academyId, item.queue_id));
  }
  function mirrorPendingToGenericQueue(tableName, payload, error) {
    ...
    const existing = queue.read(academyId).find(item => clean(item.feature) === feature && clean(item.client_mutation_id) === mutationId);
  ...
  function verifyReturnedRow(target, row, payload, mutationId) {
    ...
    if (String(row.content == null ? '' : row.content) !== String(payload.content == null ? '' : payload.content)) throw new Error(`${target.label} 서버 검증 실패: 피드백 내용이 일치하지 않습니다.`);
  ...
  function writeSyncedLocal(feature, payload, mutationId, row) {
    ...
    } catch (error) { console.warn('피드백 재전송 로컬 동기화 기록 실패:', error?.message || error); }
  }
  async function retryFeedbackQueueItem(item) {
    ...
    const payload = { ...rawPayload, academy_id: academyId, student_id: studentId, client_mutation_id: mutationId };
  ...
  function installRetryOverride() {
    ...
    const retryAll = async function retryOlliStorageQueueP0() {
      const core = global.OlliStorageCore;
      const academyId = (typeof global.settingsGetAcademyId === 'function' && global.settingsGetAcademyId()) || localStorage.getItem('olli_current_academy_id') || '';
      if (!core || !core.SyncQueue || !core.FeatureRegistry) { alert('공통 저장 기반이 아직 준비되지 않았습니다. 앱을 새로고침한 뒤 다시 시도해 주세요.'); return; }
      if (!academyId) { alert('현재 학원 ID를 확인할 수 없습니다. 학원을 다시 선택한 뒤 시도해 주세요.'); return; }
      if (navigator && navigator.onLine === false) { alert('인터넷 연결 후 재전송을 실행해 주세요.'); return; }
      ...
      for (let i = 0; i < max; i += 1) {
        ...
        if (!feature || !core.FeatureRegistry.has(feature)) {
          failed += 1;
          core.SyncQueue.update(academyId, queueId, { status: 'blocked', last_attempt_at: new Date().toISOString(), retry_count: Number(item.retry_count || 0) + 1, error_code: 'FEATURE_NOT_REGISTERED', error_message: '등록되지 않은 저장 기능입니다: ' + feature });
          continue;
        }
        if (operation === 'upload') {
          skipped += 1;
          core.SyncQueue.update(academyId, queueId, { status: 'blocked', last_attempt_at: new Date().toISOString(), retry_count: Number(item.retry_count || 0) + 1, error_code: 'FILE_RETRY_NEEDS_INDEXEDDB', error_message: '사진 파일 업로드 재전송은 IndexedDB 파일 보관 구조가 필요합니다.' });
          continue;
        }
        core.SyncQueue.update(academyId, queueId, { last_attempt_at: new Date().toISOString(), retry_count: Number(item.retry_count || 0) + 1, status: 'pending' });
        try {
          if (TARGETS[feature]) {
          ...
          if (feature === 'student_soft_delete') {
            ...
            request.data = { is_deleted: true, deleted_at: p.deleted_at || retryAt, deleted_by: p.deleted_by || localStorage.getItem('olli_current_member_id') || localStorage.getItem('olli_current_user_id') || '', delete_reason: p.delete_reason || p.reason || 'student_deleted' };
          }
          let result;
          if (operation === 'delete' || operation === 'soft_delete') {
            result = await global.deleteOlliData(feature, Object.assign({}, request, { deleteMode: (item.payload && item.payload.deleteMode) || 'soft', reason: (item.payload && item.payload.reason) || 'retry' }));
          } else {
          ...
          if ((result && result.serverSaved) || (result && result.deleted)) {
            ...
            failed += 1;
            core.SyncQueue.update(academyId, queueId, { status: 'pending', error_code: result.errorCode || 'SERVER_WRITE_PENDING', error_message: String(result.error && (result.error.message || result.error) || '서버 저장이 아직 완료되지 않았습니다.') });
          } else {
            failed += 1;
            core.SyncQueue.update(academyId, queueId, { status: 'pending', error_code: (result && result.errorCode) || 'RETRY_FAILED', error_message: String(result && result.error && (result.error.message || result.error) || (result && result.reason) || '재전송에 실패했습니다.') });
          }
        } catch (error) {
          failed += 1;
          const isFeedback = !!TARGETS[feature];
          core.SyncQueue.update(academyId, queueId, { status: isFeedback && !isRetryableError(error) ? 'blocked' : 'pending', error_code: isFeedback ? errorCode(error) : (error && error.code) || 'RETRY_FAILED', error_message: String(error && (error.message || error) || '') });
          if (core.Diagnostics && typeof core.Diagnostics.record === 'function') {
            core.Diagnostics.record({ feature, resource: '', operation: 'retry', academy_id: academyId, student_id: item.student_id || null, error_code: isFeedback ? errorCode(error) : (error && error.code) || 'RETRY_FAILED', error_message: String(error && (error.message || error) || '') });
          }
        }
      }
      if (typeof global.refreshOlliStorageDiagnostics === 'function') global.refreshOlliStorageDiagnostics();
