from pathlib import Path

path = Path('pc-timetable.css')
text = path.read_text(encoding='utf-8')
marker = '/* 출석부 표 꽉채움 + 스크롤바 외부 배치 */'
block = '''\n\n/* 출석부 표 꽉채움 + 스크롤바 외부 배치 */\n#recordRoomScreen .olliTtAttendanceRegister {\n  min-width: 0;\n  min-height: 0;\n  padding: 0 10px 10px 0;\n  border: 0;\n  border-radius: 0;\n  overflow: auto;\n  background: transparent;\n  box-shadow: none;\n  scrollbar-gutter: stable;\n}\n#recordRoomScreen .olliTtAttendanceRegisterHead {\n  flex: 0 0 auto;\n}\n#recordRoomScreen .olliTtAttendanceRegisterScroll {\n  width: max-content;\n  min-width: 100%;\n  min-height: 0;\n  padding: 0 !important;\n  flex: 0 0 auto;\n  overflow: visible !important;\n  border: 1px solid #e4e7eb;\n  border-radius: 21px;\n  background: #fff;\n  box-shadow: 0 7px 20px rgba(27,39,58,.045);\n  box-sizing: border-box;\n}\n#recordRoomScreen .olliTtAttendanceSheet {\n  width: 100%;\n  min-width: 900px;\n  margin: 0;\n  padding: 0;\n  border-radius: 20px;\n  overflow: hidden;\n  box-sizing: border-box;\n}\n#recordRoomScreen .olliTtAttendanceTable {\n  margin: 0 !important;\n}\n'''
if marker in text:
    text = text[:text.index(marker)].rstrip() + block
else:
    text = text.rstrip() + block
path.write_text(text, encoding='utf-8')

for temp in [
    Path('.github/workflows/tmp-attendance-scroll-outside.yml'),
    Path('.github/workflows/apply-attendance-scroll-fix.yml'),
    Path('.github/tmp/apply_attendance_scroll_outside.py'),
]:
    if temp.exists():
        temp.unlink()
