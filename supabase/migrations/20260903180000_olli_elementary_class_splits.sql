-- Persist elementary A/B class splits and allow safe movement between groups.

create table if not exists public.olli_schedule_class_splits (
  academy_id uuid not null references public.academies(id) on delete cascade,
  weekday smallint not null check (weekday between 1 and 6),
  time_slot smallint not null check (time_slot between 1 and 6),
  created_by_account_id uuid references public.olli_accounts(id) on delete set null,
  created_at timestamptz not null default now(),
  primary key (academy_id, weekday, time_slot)
);

alter table public.olli_schedule_class_splits enable row level security;
revoke all on table public.olli_schedule_class_splits from public, anon, authenticated;
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object('weekday', s.weekday, 'time_slot', s.time_slot) order by s.weekday, s.time_slot), '[]'::jsonb)
  into v_class_splits
  from public.olli_schedule_class_splits s
  where s.academy_id = p_academy_id;
  return jsonb_build_object(
    ...
    'changes', v_changes, 'attendance', v_attendance, 'pickups', v_pickups, 'class_splits', v_class_splits
  );
end;
$$;
...
begin
  ...
  where s.id = p_student_id and s.academy_id = p_academy_id and s.status = 'active';
  if v_division is null then return jsonb_build_object('ok', false, 'message', '학생을 찾을 수 없습니다.'); end if;
  if not private.olli_schedule_group_is_enabled(p_academy_id, v_division, v_weekday, p_time_slot) then v_class_group := 'A'; end if;
...
begin
  ...
  if v_action = 'change' then
  ...
  elsif v_action = 'remove_pickup' then
    ...
    v_result := public.olli_schedule_split_class(
      p_session_token, p_academy_id, (v_params->>'weekday')::integer, (v_params->>'time_slot')::integer
    );
  end if;
  return v_result;
...
revoke all on function private.olli_schedule_group_is_enabled(uuid, text, integer, integer) from public, anon, authenticated;
