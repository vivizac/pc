-- Dropoff pickup happens after class and does not require a separate pickup time.

alter table public.olli_schedule_pickups
  alter column pickup_time drop not null;

alter table public.olli_schedule_pickups
  drop constraint if exists olli_schedule_pickups_time_required_unless_dropoff;

alter table public.olli_schedule_pickups
  add constraint olli_schedule_pickups_time_required_unless_dropoff
  check (is_dropoff = true or pickup_time is not null);
...
declare
  ...
  v_id uuid;
  v_count integer;
  v_is_dropoff boolean := coalesce(p_is_dropoff, false);
begin
  ...
  if p_weekday not between 1 and 6
     ...
     or (not v_is_dropoff and p_pickup_time is null)
     or char_length(btrim(coalesce(p_pickup_label, ''))) not between 1 and 80 then
    return jsonb_build_object(
      'ok', false,
      'message', case when v_is_dropoff
        then '하원 픽업 요일과 장소를 확인해 주세요.'
        else '픽업 요일, 장소와 시간을 확인해 주세요.'
      end
    );
  end if;
  ...
  insert into public.olli_schedule_pickups (
    ...
    case when v_is_dropoff then null else p_pickup_time end,
    v_is_dropoff,
    v_effective,
    v_account_id
  )
