...
begin
  ...
  if found then
    ...
    -- Historical whole-note reversion protection applies to non-empty snapshots.
    -- An empty string reached here only after the expected revision matched, so it
    -- represents an intentional clear, not a stale device replacing a newer revision.
    if btrim(v_content) <> '' then
      select max(v.revision)
        into v_prior_same_revision
      ...
      where v.academy_id = p_academy_id
        ...
        and v.revision < v_current.revision
        and coalesce(v.content, '') = v_content;

      if v_prior_same_revision is not null then
        insert into private.olli_data_conflicts (
          academy_id, feature, student_id, entity_key,
          base_revision, server_revision, rejected_payload,
          mutation_id, account_id, device_id
          ...
          jsonb_build_object(
            ...
            'content', v_content,
            'conflict_reason', 'HISTORICAL_REVERSION_BLOCKED',
            'historical_revision', v_prior_same_revision
          ),
          v_mutation_id, v_account_id, v_device_id
        ) on conflict do nothing;
        ...
        return jsonb_build_object(
          'ok', false,
          'code', 'REVISION_CONFLICT',
          'reason', 'HISTORICAL_REVERSION_BLOCKED',
          'message', '과거 버전의 내용이 자동저장으로 다시 올라오는 것을 차단했습니다.',
          'expected_revision', v_expected,
          'server_revision', v_current.revision,
          'server_content', coalesce(v_current.content, ''),
          'server_updated_at', v_current.updated_at,
