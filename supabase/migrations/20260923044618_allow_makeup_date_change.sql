...
begin
  if not private.olli_schedule_can_access(p_session_token, p_academy_id) then
    return jsonb_build_object('ok', false, 'message', '보강 날짜를 변경할 권한이 없습니다.');
  end if;

  if p_one_time_session_id is null or p_session_date is null or p_session_date < current_date then
    return jsonb_build_object('ok', false, 'message', '변경할 보강 날짜를 확인해 주세요.');
  end if;
  ...
  if not found
     or v_item.student_id is null
     or v_item.session_type <> 'makeup'
     or v_item.status = 'cancelled' then
    return jsonb_build_object('ok', false, 'message', '변경할 보강 수업을 찾을 수 없습니다.');
  end if;
  ...
  if (v_division = 'elementary' and v_weekday = 6 and v_item.time_slot not in (10, 11, 12))
    ...
    return jsonb_build_object('ok', false, 'message', '선택한 날짜에는 현재 보강 시간으로 수업할 수 없습니다.');
  end if;
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    p_academy_id::text || ':makeup-student:' || v_item.student_id::text || ':' || p_session_date::text || ':' || v_item.time_slot::text,
    0
  ));
  ...
  if exists (
    ...
    return jsonb_build_object('ok', false, 'message', '같은 날짜와 시간에 이미 등록된 보강이 있습니다.');
  end if;
  ...
  perform set_config('olli.schedule_action', 'makeup_date_change', true);
