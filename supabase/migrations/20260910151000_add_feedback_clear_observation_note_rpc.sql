create or replace function public.olli_note_draft_clear_after_feedback(
...
begin
  ...
  if v_account_id is null or not exists (
    ...
    return jsonb_build_object('ok', false, 'code', 'PERMISSION_DENIED', 'message', '관찰노트를 초기화할 권한이 없습니다.');
  end if;

  if p_student_id is null or v_note_type = '' then
    return jsonb_build_object('ok', false, 'code', 'INVALID_INPUT', 'message', '관찰노트 초기화 정보를 확인해 주세요.');
  end if;
  ...
  if v_mutation_id is not null and v_current.last_mutation_id = v_mutation_id then
    return jsonb_build_object(
      'ok', true,
      'idempotent', true,
      'no_change', coalesce(v_current.content, '') = '',
      'intentional_clear', true,
  ...
  if coalesce(v_current.content, '') = '' then
    return jsonb_build_object(
      'ok', true,
      'idempotent', false,
      'no_change', true,
  ...
  if v_current.revision <> v_expected then
    insert into private.olli_data_conflicts (
      ...
      jsonb_build_object(
        'note_type', v_note_type,
        'content', '',
        'conflict_reason', 'FEEDBACK_CLEAR_REVISION_CONFLICT'
      ),
    ...
    return jsonb_build_object(
      'ok', false,
      'code', 'REVISION_CONFLICT',
      'reason', 'FEEDBACK_CLEAR_REVISION_CONFLICT',
      'message', '피드백 저장 후 다른 기기에서 관찰노트가 변경되었습니다. 메모는 지우지 않았습니다.',
      'expected_revision', v_expected,
  ...
  update public.student_note_drafts
     set content = '',
         ...
         updated_at = now(),
         last_mutation_id = coalesce(v_mutation_id, 'feedback_clear_' || gen_random_uuid()::text),
  ...
  return jsonb_build_object(
    ...
    'student_id', v_current.student_id,
    'note_type', v_current.note_type,
    'mutation_id', v_current.last_mutation_id
  );
end;
$function$;

revoke all on function public.olli_note_draft_clear_after_feedback(text, uuid, uuid, text, bigint, text, text) from public;
grant execute on function public.olli_note_draft_clear_after_feedback(text, uuid, uuid, text, bigint, text, text) to anon, authenticated;
